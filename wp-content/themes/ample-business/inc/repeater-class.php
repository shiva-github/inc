...
if (class_exists('WP_Customize_control')) {
    ...
    class Ample_Business_Repeater_Controler extends WP_Customize_Control
    {
        /**
         * The control type.
         *
        ...
        public $ample_business_box_label = '';

        public $ample_business_box_add_control = '';
        ...
        {
            $this->fields = $fields;
            $this->ample_business_box_label = $args['ample_business_box_label'];
            $this->ample_business_box_add_control = $args['ample_business_box_add_control'];
        ...
        <?php } ?>
            ...
            <ul class="ample_business-repeater-field-control-wrap">
                <?php
                $this->ample_business_get_fields();
                ?>
            </ul>

            <input type="hidden" <?php $this->link(); ?> class="ample_business-repeater-collector"
                   value="<?php echo esc_attr($this->value()); ?>"/>
            <button type="button"
                    class="button ample_business-add-control-field"><?php echo esc_html($this->ample_business_box_add_control); ?></button>
            <?php
        }

        private function ample_business_get_fields()
        {
            $fields = $this->fields;
            $values = json_decode($this->value());
            ...
            if (is_array($values)) {
                foreach ($values as $value) {
                    ?>
                    <li class="ample_business-repeater-field-control">
                        <h3 class="ample_business-repeater-field-title"><?php echo esc_html($this->ample_business_box_label); ?></h3>

                        <div class="ample_business-repeater-fields">
                            <?php
                            foreach ($fields as $key => $field) {
                                $class = isset($field['class']) ? $field['class'] : '';
                                ?>
                                <div
                                        class="ample_business-fields ample_business-type-<?php echo esc_attr($field['type']) . ' ' . $class ?>">
                                    ...
                                    switch ($field['type']) {
                                        ...
                                        case 'upload':
                                            ...
                                            echo '<div class="ample_business-fields-wrap">';
                                            echo '<div class="attachment-media-view">';
                                            echo '<div class="placeholder' . $image_class . '">';
                                            esc_html__('No image selected', 'ample-business');
                                            echo '</div>';
                                            echo '<div class="thumbnail thumbnail-image">';
                                            echo $image;
                                            echo '</div>';
                                            echo '<div class="actions clearfix">';
                                            echo '<button type="button" class="button ample_business-delete-button align-left">' . esc_html__('Remove', 'ample-business') . '</button>';
                                            echo '<button type="button" class="button ample_business-upload-button alignright">' . esc_html__('Select Image', 'ample-business') . '</button>';
                                        ...
                                        case 'category':
                                            ...
                                            echo '<option value="0">' . esc_html__('Select Category', 'ample-business') . '</option>';
                                            echo '<option value="-1">' . esc_html__('Latest Posts', 'ample-business') . '</option>';
                                        ...
                                        case 'colorpicker':
                                            echo '<input data-default="' . esc_attr($default) . '" class="ample_business-color-picker" data-alpha="true" data-name="' . esc_attr($key) . '" type="text" value="' . esc_attr($new_value) . '"/>';
                                            break;
                                        ...
                                        case 'range':
                                            ...
                                            echo '<div class="ample_business-range-slider" >';
                                        ...
                                        case 'icon':
                                            echo '<div class="ample_business-selected-icon">';
                                            echo '<i class="' . esc_attr($new_value) . '"></i>';
                                            echo '<span><i class="fa fa-angle-down"></i></span>';
                                            echo '</div>';
                                            echo '<ul class="ample_business-icon-list clearfix">';
                                            ...
                                            foreach ($ample_business_font_awesome_icon_array as $ample_business_font_awesome_icon) {
                                                ...
                                                echo '<li class=' . $icon_class . '><i class="' . $ample_business_font_awesome_icon . '"></i></li>';
                                            }
                                            echo '</ul>';
                                            echo '<input data-default="' . esc_attr($default) . '" type="hidden" value="' . esc_attr($new_value) . '" data-name="' . esc_attr($key) . '"/>';
                                            break;
                                        ...
                                        case 'multicategory':
                                            ...
                                            echo '<ul class="ample_business-multi-category-list">';
                                            echo '<li><label><input type="checkbox" value="-1" ' . checked('-1', $new_value, false) . '/>' . esc_html_e('Latest Posts', 'ample-business') . '</label></li>';
                                            foreach ($this->cats as $cat) {
                            ...
                            <div class="clearfix ample_business-repeater-footer">
                                <div class="alignright">
                                    <a class="ample_business-repeater-field-remove"
                                       href="#remove"><?php esc_html_e('Delete', 'ample-business') ?></a> |
                                    <a class="ample_business-repeater-field-close"
                                       href="#close"><?php esc_html_e('Close', 'ample-business') ?></a>
                                </div>
                            </div>
                        </div>
                    </li>
...
if (!function_exists('ample_business_font_awesome_icon_array')) {
    function ample_business_font_awesome_icon_array()
    {
